...
class GMM():
    ...
    def _do_e_step(self, X):
        ...
        for k in range(self.n_components):
            r_numerator[:,k] = self.weigths_[k]*sc.stats.multivariate_normal.pdf(
    ...
    def _do_m_step(self, X, responsibility):
        ...
        for k in range(self.n_components):
            ...
            for n in range(n_rows):
                ...
                cov_numerator[n,:,:] = responsibility[n,k]*np.dot(
                    aux[:,None],aux[:,None].T)
    ...
    def fit(self, X, y=None):
        ...
        self.weigths_ = sc.stats.dirichlet.rvs(np.ones(shape=self.n_components))[0]
        ...
        for i in range(self.n_iter):
            ...
            # E-step
            responsibilities = self._do_e_step(X)

            # M-step
            self._do_m_step(X, responsibilities)
